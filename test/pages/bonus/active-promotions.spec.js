...
import MyPromotionsPage from "@/pages/bonus/active-promotions"
...
describe('promoBlockPage component test', () => {
  ...
  test('promotions block component test', () => {
    ...
    const a = component.find('.promotions-page-block a'); //finds an `a` element
...
describe('Page: my promotions', () => {
  test('testing page promotions', () => {
    ...
    const storeKey = `promotions/${GET_MY_PROMOTIONS_INFO_BLOCKS}`
    const page = shallowMount(MyPromotionsPage, {
      mocks: {
        $store: {
          getters: {
            [storeKey]: promoCards
    ...
    expect(pageTitle.text()).toBe('Promotions')
